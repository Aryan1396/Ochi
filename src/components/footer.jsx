...
function footer() {
    return (
        <div  className='container w-full h-screen  bg-white mt[-10vw]'>

            <div className='flex justify-between p-20'>
                <div className=''>
                    <h1 className='text-[7vw] uppercase tracking-tighter font-[700] leading-none'>eye-opening</h1>
                </div>
                <div>
                    <h1 className='text-[7vw] uppercase tracking-tighter  flex ml-[150px] font-[700] leading-none'>PRESENTATIONS</h1>
                    <div className='flex flex-col gap-8 ml-[150px] mt-10 text-[17px] h-[30vw] flex-wrap'>
                        <div className='flex flex-col'>
                            <h1>S:</h1>
                          
                            {["Instagram","Behance","Facebook","Linkedin"].map((item, index)=>(
                                <a className={`flex flex-col underline ${index==0 && "mt-[10px]"}`} key={index} href="">{item}</a>
                            ))}
                        </div>
                        <div className='flex flex-col '>
                            <h1>L:</h1>
                         
                            <a href="" className=' w-[200px] pt-[20px]'>
                                </a>
                                {["202-1965 W 4th Ave Vancouver, Canada", "30 Chukarina St Lviv, Ukraine"].map((item,index)=>(
                                    <a key={index} className={`flex flex-col w-[200px] underline ${index==1 && "mt-[10px]"}`} href="">{item}</a>
                                ))}
                        </div>
                        <div className='flex flex-col '>
                            <h1>E:</h1>
                            <a className='mt-4 underline' href="">hello@ochi.design</a>
                        </div>
                        <div className='flex flex-col pt-[160px] pl-[300px]'>
                            <h1>M:</h1>
                            {["Home","Services","Our work","About us","Insights","Contact us"].map((item,index)=>(
                                <a key={index} className={`underline ${index==0 && "mt-[10px]"}`} href="">{item}</a>
            ...
            <div className='footer flex justify-between px-10 -pt-20 pb-5'>
               <div>
               <svg width="72" height="30" viewBox="" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div>
                   <p className='text-[#c2c2c2] font-[400] ml-[150px]'>© ochi design 2024. Legal Terms</p> 
                </div>
                <div>
                    <a href="" className='text-[#c2c2c2] font-[400]'>Website by Obys</a>
                </div>
            </div>
        </div>
    )
